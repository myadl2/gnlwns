 ...
 * 
 * 변수란?
 * - 값을 저장할 수 있는 메모리(컴퓨터가 인식할수 있는 영역)의 공간
 * - 변수는 오직 하나의 값만을 담을 수 있다.
 * - 프로그램에서 변수를 사용하려면?
 *  : 변수의 선언, 값을 대입
 *  : 타입 이름;   이름 = 값;
 * - 값을 변수에 대입할 때는 "="을 사용한다.
 * - 변수의 선언과 값의 대입을 한 번에 할 수 있다.
 *  : 타입 이름  = 값;  
 *  
 *  참고
 *  int: 숫자 중에서 정수를 담는다. 
 *  String : 문자가 여러개 모인 문자열을 담는 데이터 타입. 문자열을 표현 할때는 ""로 묶는다.
 *  
 */
package ch01_basic;
...
//		System.out.println(i); //변수 선언없이 사용할 때 에러 발생
	...
	값이 대입이 안 된 경우.*/
		...
		System.out.println(i); //화면에 i란 이름의 변수공간에 담긴 값을 출력한다.
		
		//변수k를 만들고 k의 공간에 홍길동을 대입한다.
		//화면에 k변수에 대입된 값을 출력한다.
